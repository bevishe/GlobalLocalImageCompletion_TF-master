...
class network():
    def __init__(self, args):
        '''
        network对像的__init__函数
        :param args:从config模块接受过来的关于定义网络结构的参数，相当于是个字典
        '''
        ...
        self.real_img, self.perturbed_img, self.mask, self.coord, self.pads, self.data_count = ops.load_train_data(args)
    ...
    def build_model(self):
        ...
        self.fake_loss = architecture.linear(self.fake_d_logits, 1, "fake_loss")
        self.real_loss = architecture.linear(self.real_d_logits, 1, "real_loss")
    ...
    def completion_net(self, input, name="generator", reuse=False):
        ...
        nets = []
        # 使用了architecture.py文件用来设置网络的结构
        with tf.variable_scope(name, reuse=reuse) as scope:
            conv1 = architecture.conv2d(input, 64,
                          kernel=5,
                          stride=1,
                          padding="SAME",
                          name="conv1"
            ...
            conv2 = architecture.conv2d(conv1, 128,
                          kernel=3,
                          stride=2,
                          padding="SAME",
                          name="conv2"
            ...
            conv3 = architecture.conv2d(conv2, 128,
                          kernel=3,
                          stride=1,
                          padding="SAME",
                          name="conv3"
                          )
            conv3 = architecture.batch_norm(conv3, name="conv_bn3")
            conv3 = tf.nn.relu(conv3)

            conv4 = architecture.conv2d(conv3, 256,
                          kernel=3,
                          stride=2,
                          padding="SAME",
                          name="conv4"
            ...
            conv5 = architecture.conv2d(conv4, 256,
                          kernel=3,
                          stride=1,
                          padding="SAME",
                          name="conv5"
            ...
            conv6 = architecture.conv2d(conv5, 256,
                          kernel=3,
                          stride=1,
                          padding="SAME",
                          name="conv6"
                          )
            conv6 = architecture.batch_norm(conv5, name="conv_bn6")
            conv6 = tf.nn.relu(conv5)

            #Dilated conv from here
            dilate_conv1 = architecture.dilate_conv2d(conv6, 
                                        [self.batch_size, conv6.get_shape()[1], conv6.get_shape()[2], 256],
                                        rate=2,
                                        name="dilate_conv1")

            dilate_conv2 = architecture.dilate_conv2d(dilate_conv1, 
                                        [self.batch_size, dilate_conv1.get_shape()[1], dilate_conv1.get_shape()[2], 256],
                                        rate=4,
                                        name="dilate_conv2")

            dilate_conv3 = architecture.dilate_conv2d(dilate_conv2, 
                                        [self.batch_size, dilate_conv2.get_shape()[1], dilate_conv2.get_shape()[2], 256],
                                        rate=8,
                                        name="dilate_conv3")

            dilate_conv4 = architecture.dilate_conv2d(dilate_conv3, 
                                        [self.batch_size, dilate_conv3.get_shape()[1], dilate_conv3.get_shape()[2], 256],
                                        rate=16,
                                        name="dilate_conv4")                                                                                              

            #resize back
            conv7 = architecture.conv2d(dilate_conv4, 256,
                          kernel=3,
                          stride=1,
                          padding="SAME",
                          name="conv7"
            ...
            conv8 = architecture.conv2d(conv7, 256,
                          kernel=3,
                          stride=1,
                          padding="SAME",
                          name="conv8"
            ...
            deconv1 = architecture.deconv2d(conv8, [self.batch_size, int(input_shape[1]/2), int(input_shape[2]/2), 128], name="deconv1")
            deconv1 = architecture.batch_norm(deconv1, name="deconv_bn1")
            deconv1 = tf.nn.relu(deconv1)

            conv9 = architecture.conv2d(deconv1, 128,
                          kernel=3,
                          stride=1,
                          padding="SAME",
                          name="conv9"
                          )
            conv9 = architecture.batch_norm(conv9, name="conv_bn9")
            conv9 = tf.nn.relu(conv9)

            deconv2 = architecture.deconv2d(conv9, [self.batch_size, input_shape[1], input_shape[2], 64], name="deconv2")
            deconv2 = architecture.batch_norm(deconv2, name="deconv_bn2")
            deconv2 = tf.nn.relu(deconv2)

            conv10 = architecture.conv2d(deconv2, 32,
                          kernel=3,
                          stride=1,
                          padding="SAME",
                          name="conv10"
                          )
            conv10 = architecture.batch_norm(conv10, name="conv_bn10")
            conv10 = tf.nn.relu(conv10)

            conv11 = architecture.conv2d(conv10, 3,
                          kernel=3,
                          stride=1,
                          padding="SAME",
                          name="conv11"
                          )
            conv11 = architecture.batch_norm(conv11, name="conv_bn11")
    ...
    def local_discriminator(self, input, name="local_discriminator", reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse) as scope:
            conv1 = tf.contrib.layers.conv2d(input, 64, 5, 2,
                                     ...
                                     activation_fn=None,
                                     scope="conv1")
            conv1 = architecture.batch_norm(conv1, name="bn1")
            conv1 = tf.nn.relu(conv1)
            nets.append(conv1)
            ...
            conv2 = tf.contrib.layers.conv2d(conv1, 128, 5, 2,
                                     ...
                                     activation_fn=None,
                                     scope="conv2")
            conv2 = architecture.batch_norm(conv2, name="bn2")
            conv2 = tf.nn.relu(conv2)
            nets.append(conv2)
            ...
            conv3 = tf.contrib.layers.conv2d(conv2, 256, 5, 2,
                                     ...
                                     activation_fn=None,
                                     scope="conv3")
            conv3 = architecture.batch_norm(conv1, name="bn3")
            conv3 = tf.nn.relu(conv3)
            nets.append(conv3)
            ...
            conv4 = tf.contrib.layers.conv2d(conv3, 512, 5, 2,
                                     ...
                                     activation_fn=None,
                                     scope="conv4")
            conv4 = architecture.batch_norm(conv4, name="bn4")                                                                                                                           
            conv4 = tf.nn.relu(conv4)
            nets.append(conv4)
            ...
            conv5 = tf.contrib.layers.conv2d(conv4, 512, 5, 2,
                                     ...
                                     activation_fn=None,
                                     scope="conv5")
            conv5 = architecture.batch_norm(conv5, name="bn5")                                                                                                                           
            conv5 = tf.nn.relu(conv5)
            nets.append(conv5)

            flatten = tf.contrib.layers.flatten(conv5)

            output = architecture.linear(flatten, 1024, name="linear")
    ...
    def global_discriminator(self, input, name="global_discriminator", reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse) as scope:
            ...
            conv6 = tf.contrib.layers.conv2d(conv5, 512, 5, 2,
                                     ...
                                     activation_fn=None,
                                     scope="conv6")
            conv6 = architecture.batch_norm(conv6, name="bn6")                                                                                                                           
